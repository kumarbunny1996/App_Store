...
export class PostEffects {
    ...
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.postService.getPosts().pipe(
          map((resObj) =>
            FromPostActions.loadPostsSuccess({ posts: resObj.listArr })
          ),
          catchError((error) =>
            of(FromPostActions.loadPostsFailure({ error: error.error }))
          )
        )
      )
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.postService.addPost(action.post).pipe(
          map((resObj) =>
            FromPostActions.addPostSuccess({ post: resObj.postData })
          ),
          catchError((error) =>
            of(FromPostActions.addPostFailure({ error: error.error }))
          )
        )
      )
    )
  ...
  loadPost$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.postService.getPost(action.ROWID).pipe(
          map((resObj) =>
            FromPostActions.loadPostSuccess({
              selectedPost: resObj.selectedPost,
            })
          ),
          catchError((error) =>
            of(FromPostActions.loadPostFailure({ error: error.error }))
          )
        )
      )
    )
  ...
  updatePost$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        concatMap((action) =>
          this.postService.updatePost(action.post.id, action.post.changes)
        ),
        tap(
